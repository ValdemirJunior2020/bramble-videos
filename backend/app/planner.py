...
MASTER_NEGATIVE = (
    ...
    "wrong clothing, swapped clothing, mixed clothing, face blend, merged face, merged character, "
    "text, watermark, signature, scary, aggressive, hyper-saturated, neon colors, glowing eyes, "
    "blurry, low quality, low resolution, jpeg artifacts, flat lighting, random unrelated person, "
    "wrong environment, old church art, statue, icon, manuscript, character sheet, turnaround sheet, "
...
def _is_object_or_environment_focus(narration: str) -> bool:
    ...
    action_hits = sum(1 for word in ["walked", "ran", "jumped", "pulled", "grabbed", "looked", "smiled"] if word in lower)
        ...
        "messages": [
            {
                ...
                "content": (
                    ...
                    "Preserve character identity, clothing, colors, and continuity. Avoid more than two active characters unless clearly required by the narration."
                ),
            },
            {
                "role": "user",
...
def _traits(names: list[str]) -> str:
    ...
    for name in names:
        ...
        parts.append(
            f"{asset.name}: keep exact approved face, body type/species, hairstyle or fur, outfit, clothing colors, and proportions from uploaded references; {detail}"
        )
    return "; ".join(parts)
...
def build_prompt(scene: Scene) -> str:
    ...
    if scene.characters:
        ...
        parts.append(
            "Never merge characters. Never swap clothing. Never give one character another character's face, hair, fur, body, or outfit. "
            "Keep every named character separate, recognizable, and in their correct approved clothing."
        )
    else:
