...
if errorlevel 1 (
  if exist "chatterbox_service\.venv\Scripts\python.exe" (
    echo Starting Bramble expressive voice service...
    start "Bramble Chatterbox" /D "%~dp0chatterbox_service" cmd /k ".\.venv\Scripts\python.exe -m uvicorn app:app --host 127.0.0.1 --port %CHATTERBOX_PORT%"
  ) else (
